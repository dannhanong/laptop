...
import com.mshop.entity.StatisticalYears;
...
public class StatisticalRestApi {
	...
	@GetMapping("years")
	public ResponseEntity<List<StatisticalYears>> getStatisticalAllYear() {
		List<Object[]> list = repo.getStatisticalYear();
		List<StatisticalYears> listSta = new ArrayList<>();
		for(int i = 0; i<list.size(); i++) {
			StatisticalYears sta = new StatisticalYears((int) list.get(i)[0], (Double) list.get(i)[1], (BigInteger) list.get(i)[2]);
			listSta.add(sta);
		}
		return ResponseEntity.ok(listSta);
	}
